...
struct GlobalMemory {
  ...
  unsigned long done;
  long id;
  // Add padding to ensure each array is aligned to a cache line boundary
  alignas(64) char padding1[64];
...
struct LocalCopies {
  ...
  double t_in_mod;
  double t_in_bar;
  // Add padding to ensure each double is aligned to a cache line boundary
